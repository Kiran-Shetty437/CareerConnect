...
"""
        ...
        for model_name in model_list:
            try:
                ...
                if "503" in last_error or "high demand" in last_error.lower():
                    print(f"Model {model_name} is under high demand. Falling back...")
                    continue
                else:
                    raise e
                    
        if not response:
            raise ValueError(f"All models failed: {last_error}")
        ...
        # MERGE LOGIC: Use original pattern as the base to ensure matching rounds and times
        final_test = []
        for idx, s_conf in enumerate(sections):
            section_name = s_conf.get("section", f"Round {idx+1}")
            minutes = s_conf.get("minutes", 10) # Fallback to 10 only if pattern is broken
            
            # Find the matching section generated by AI
            ai_section = None
            if idx < len(generated_test):
                ai_section = generated_test[idx]
            
            final_test.append({
                "section": section_name,
                "minutes": minutes,
                "questions": ai_section.get("questions", []) if ai_section else []
            })

        # Save successfully generated questions to DB for future fallback
        if final_test:
            save_questions_to_db(final_test, difficulty, company_name)
